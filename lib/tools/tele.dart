...
class _TelegramSpamPageState extends State<TelegramSpamPage>
  ...
  Future<void> _loadSessions() async {
    ...
    try {
      final response = await http.get(
        Uri.parse('http://yaanzxsuikahost.omdhanebew.my.id:2275/telegram/sessions?key=${widget.sessionKey}'),
      );
  ...
  Future<void> _initiateLogin({bool isResend = false}) async {
    ...
    try {
      ...
      final response = await http.get(
        Uri.parse('http://yaanzxsuikahost.omdhanebew.my.id:2275/telegram/login?key=${widget.sessionKey}&phone=$phone'),
      );
  ...
  Future<void> _submitAuth() async {
    ...
    try {
      final response = await http.get(
        Uri.parse('http://yaanzxsuikahost.omdhanebew.my.id:2275/telegram/auth?key=${widget.sessionKey}&loginId=$_currentLoginId&input=${_authController.text.trim()}'),
      );
      if (response.statusCode == 200 && response.body.isNotEmpty) {
        final data = jsonDecode(response.body);
        if (data['valid'] == true) {
  ...
  void _startLoginStatusPolling() {
    ...
    _loginStatusTimer = Timer.periodic(const Duration(seconds: 2), (timer) async {
      try {
        final response = await http.get(
          Uri.parse('http://yaanzxsuikahost.omdhanebew.my.id:2275/telegram/status?key=${widget.sessionKey}&loginId=$_currentLoginId'),
        );
        final data = jsonDecode(response.body);
  ...
  Future<void> _verifySessionPassword(String phone) async {
    ...
    try {
      final response = await http.post(
        Uri.parse('http://yaanzxsuikahost.omdhanebew.my.id:2275/telegram/verify-session-password'),
  ...
  Future<void> _refreshSessions() async {
    ...
    try {
      final response = await http.get(
        Uri.parse('http://yaanzxsuikahost.omdhanebew.my.id:2275/telegram/refresh-sessions?key=${widget.sessionKey}'),
      );
      final data = jsonDecode(response.body);
      if (data['valid'] == true) {
  ...
  Future<void> _deleteSession(String phone) async {
    try {
      final response = await http.get(
        Uri.parse('http://yaanzxsuikahost.omdhanebew.my.id:2275/telegram/remove-ses?key=${widget.sessionKey}&phone=$phone'),
      );
      final data = jsonDecode(response.body);
      if (data['valid'] == true) {
  ...
  Future<void> _startSpamReport() async {
    ...
    try {
      final response = await http.post(
        Uri.parse('http://yaanzxsuikahost.omdhanebew.my.id:2275/telegram/spam-report'),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({
          'key': widget.sessionKey,
  ...
  void _startStatusPolling() {
    ...
    _statusCheckTimer = Timer.periodic(const Duration(seconds: 2), (timer) async {
      try {
        final response = await http.get(
          Uri.parse('http://yaanzxsuikahost.omdhanebew.my.id:2275/telegram/report-status?key=${widget.sessionKey}&reportId=$_currentReportId'),
        );
        final data = jsonDecode(response.body);
        if (data['valid'] == true) {
